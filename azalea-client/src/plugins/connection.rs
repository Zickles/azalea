...
use thiserror::Error;
use tokio::{ io::AsyncWriteExt, net::tcp::OwnedWriteHalf, sync::mpsc::{ self } };
use tracing::{ debug, error, info, trace };

use super::packet::{
    config::ReceiveConfigPacketEvent,
    game::ReceiveGamePacketEvent,
        ...
        loop {
            ...
            match read_res {
                Ok(Some(raw_packet)) => {
                    ...
                    conn.tapped_server_packets.push(raw_packet.to_vec().into_boxed_slice());
                    if
                        let Err(e) = handle_raw_packet(
                            ecs,
                            &raw_packet,
...
pub struct RawConnection {
    ...
    pub(crate) tapped_server_packets: Vec<Box<[u8]>>,
}
impl RawConnection {
    pub fn new(
        ...
        let writer_task = task_pool.spawn(
            write_task(network_packet_writer_rx, writer.write_stream)
        );
    ...
    pub fn new_networkless(state: ConnectionProtocol) -> Self {
        Self {
            ...
            tapped_server_packets: Vec::new(),
        }
    }

    pub fn take_tapped_packets(&mut self) -> Vec<Box<[u8]>> {
        std::mem::take(&mut self.tapped_server_packets)
    }
...
pub fn handle_raw_packet(
    ...
    match state {
        ...
        ConnectionProtocol::Game => {
            ...
            queued_packet_events.game.push(ReceiveGamePacketEvent { entity, packet });
        }
        ...
        ConnectionProtocol::Login => {
            ...
            queued_packet_events.login.push(ReceiveLoginPacketEvent { entity, packet });
        }
        ConnectionProtocol::Configuration => {
            let packet = Arc::new(deserialize_packet::<ClientboundConfigPacket>(stream)?);
            trace!("Packet: {packet:?}");
            config::process_packet(ecs, entity, &packet);
            queued_packet_events.config.push(ReceiveConfigPacketEvent { entity, packet });
        }
    }
